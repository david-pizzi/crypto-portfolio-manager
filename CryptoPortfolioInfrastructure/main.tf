...
resource "azurerm_linux_function_app" "crypto_function" {
  ...
  app_settings = {
    ...
    "CosmosDBConnectionString"                  = azurerm_cosmosdb_account.crypto_portfolio.primary_sql_connection_string
    "IntervalMinutes"                           = "5"
...
resource "azurerm_app_service" "crypto_portfolio_container" {
  ...
  app_settings = {
    ...
    # CosmosDb
    COSMOSDB__CONNECTIONSTRING           = azurerm_cosmosdb_account.crypto_portfolio.primary_sql_connection_string
